...
<header class="fixed-top">
            ...
            <div class="container-fluid p-3">
                ...
                <div class="collapse navbar-collapse justify-content-end" id="navbarSupportedContent">
                    <ul class="navbar-nav gap-2 mb-2" id="navbarSupportedContent">
                        ...
                        <li class="nav-item">
                            <a class="nav-link rounded active" aria-current="page" href="index.php?#forYou">Home</a>
                        </li>
                        ...
                        <li class="nav-item">
                            <a class="nav-link disabled" href="#">New Tourism</a>
                        </li>
    ...
    <?php if(isset($_GET['place'])): 
        $place = $_GET['place'];
        $query = mysqli_query($conn, "SELECT * from tourism where name = '$place'");
        $data = mysqli_fetch_array($query);
        $rating = $data['rating'];
        $price = $data['price'];
        $place_id = $data['id'];
                ...
                <div class="card shadow-lg">
                        ...
                        <form action="process.php" method="post">
                            <center>
                                ...
                                <div class="ratings-wrapper mb-1">
                                    ...
                                    <?php if(isset($_SESSION['username'])): ?>
                                        <input type="hidden" name="username" value="<?=$username?>" id="username"value="<?=$username?>">
                                    <?php endif; ?>
                    ...
                    <div class="overflow-auto">
                        <div class="card-body">
                            <h3>Comments</h3>
                            <?php 
                            $query = mysqli_query($conn, "select f.id, f.username, f.comment, f.rating, a.image from feedback f inner join account a on a.username = f.username where f.place_id = $place_id");
                            while($feedback = mysqli_fetch_array($query)) :
                        ?>
                            <div class="card mb-3 ">
                                <div class="card-header d-flex align-items-center gap-3 ">
                                    <?php if($feedback['image']): ?>
                                    <img src="img/profile/<?=$feedback['image']?>" class="rounded-circle me-2" alt="..."
                                        style="width:50px; height: 50px;">
                                    <?php else: ?>
                                    <img class="rounded-circle" alt="PP" src="img/profile.png"
                                        style="width:50px; height: 50px;">
                                    <?php endif; ?>

                                    <div class="feedback-username"><?= $feedback['username'] ?></div>
                                    <div class="d-flex gap-2 feedback-rating">
                                        <i class="bi bi-star-fill" style="color:#ffc107;"></i>
